...
func kolej():
	...
	if TWOJA_KOLEJ == 1:
		...
		for x in gracz2:
			if stos.is_empty():
				push_error("Próba pobrania z pustej tablicy w kolej()")
				return  # logika awaryjna
...
var can_click_playagain = true

func _on_play_again_button_pressed():
	if not can_click_playagain:
		return
	else:
		can_click_playagain = false
		_ready()
		$VBoxContainer.visible = false
		await get_tree().create_timer(2).timeout
		can_click_playagain = true


func _on_leave_game_button_pressed():
	get_tree().quit()

var ekran = DisplayServer.window_get_mode()
